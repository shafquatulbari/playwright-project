...
| ------------- | ----------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| TS_001_TC_001 | None                    | 1) Open UI 2) Switch to Register 3) Enter name, unique email, password 4) Submit | Board loads. Header shows “Signed in as`<name>`”.                                            |
| TS_001_TC_002 | Already Registered User | 1) Logout 2) Switch to Login 3) Enter same credentials 4) Submit                 | Board loads with empty columns or filled columns depending on status of user (Todo/Doing/Done). |
...
| Test Case ID  | Pre-conditions         | Steps                                                                 | Expected Result                                                     |
| ------------- | ---------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------- |
| TS_003_TC_001 | Logged in, Item exists | 1) PUT /api/items/:id to change title/priority 2) Click Refresh in UI | UI shows updated title/priority; updatedAt increases in UI and API. |

### TS_004: Delete Item and Verify in API

| Test Case ID  | Pre-conditions         | Steps                       | Expected Result                                                                |
| ------------- | ---------------------- | --------------------------- | ------------------------------------------------------------------------------ |
| TS_004_TC_001 | Logged in, Item exists | 1) Click Delete on the card | API no longer returns the item; UI count decrements; item removed from column. |

### TS_005: Move Item Across Columns

| Test Case ID  | Pre-conditions              | Steps                   | Expected Result                                                                                        |
| ------------- | --------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------ |
| TS_005_TC_001 | Logged in, 2+ items in Todo | 1) Drag a card to Doing | UI shows card in Doing; GET /api/items shows column='doing'; counts match in Doing and adjust in Todo. |

### TS_006: Reorder Items Within Column

| Test Case ID  | Pre-conditions                  | Steps                                 | Expected Result                                                                          |
| ------------- | ------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------- |
| TS_006_TC_001 | Logged in, 2+ items in a column | 1) Drag to reorder within same column | API order fields (1..N) reflect visual order; UI order equals API order for that column. |
...
| Test Case ID  | Pre-conditions | Steps                          | Expected Result                                         |
| ------------- | -------------- | ------------------------------ | ------------------------------------------------------- |
| TS_008_TC_001 | Existing user  | 1) Login with wrong password   | UI error; API 401.                                      |
| TS_008_TC_002 | Logged in      | 1) Try to add with empty title | UI prevents or shows error; API 400 if called directly. |
|               |                |                                |                                                         |
